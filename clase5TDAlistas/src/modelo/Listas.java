...
public class Listas implements IListas{
	...
	public void agregarPrimero(int d) {
		//lista --> |22| --> |3| --> |4|
		//crear nodo
		INodo nuevoNodo = new Nodo(d);//dato: 44, siguiente: null.
		if (!estaVacia()) {
			nuevoNodo.setSiguiente(this.primero);//vincular al 44 con el 22
			//(44) --> |22| --> |3| --> |4|
	...
	public void agregarUltimo(int d) {
		///Listas --> |11| --> |22| --> |33| --> null. se quiere agregar el (44) -->null
		INodo nuevoNodo = new Nodo(d);//dato: 44, siguiente: null.
		if (!estaVacia()) {
			INodo actual = this.primero;
			while (actual.getSiguiente() != null) {//Se busca un null en la pos siguiente, indicando que es el último elemento
				actual = actual.getSiguiente();
				}
			//Actual será el último elemento
